...
const OJ_POLL_INTERVAL_MS: &str = "5";
...
fn binary_path(name: &str) -> PathBuf {
    ...
    // Fall back to standard target directory
    ...
    fn new() -> Self {
        Self {
            ...
            envs: vec![
                ...
                ("OJ_POLL_INTERVAL_MS".into(), OJ_POLL_INTERVAL_MS.into()),
            ],
        }
    }
...
impl Project {
    ...
    pub fn oj(&self) -> CliBuilder {
        cli()
            .pwd(self.path())
            .env("XDG_STATE_HOME", self.state_path())
            .env("OJ_SOCKET_DIR", self.state_path())
    }
}
