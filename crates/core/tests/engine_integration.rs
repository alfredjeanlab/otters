...
use oj_core::engine::{Engine, RecoveryConfig, Scheduler, ScheduledKind};
...
async fn engine_full_pipeline_lifecycle() {
    ...
    let pipeline = engine.get_pipeline(&PipelineId("pipe-1".to_string())).unwrap();
    assert_eq!(pipeline.phase.name(), "init");

    // Transition through phases
    engine
        .process_pipeline_event(&PipelineId("pipe-1".to_string()), PipelineEvent::PhaseComplete)
        .await
        .unwrap();
...
async fn engine_task_completion_cascades_to_pipeline() {
    ...
    engine
        .process_task_event(
            &TaskId("task-1".to_string()),
            TaskEvent::Complete { output: Some("done".to_string()) },
        )
        .await
        .unwrap();
...
async fn engine_stuck_task_triggers_nudge() {
    ...
    let has_send = calls.iter().any(|c| matches!(c, oj_core::adapters::fake::AdapterCall::SendToSession { .. }));
...
fn scheduler_init_defaults_creates_standard_timers() {
    ...
    assert!(ready.len() >= 2, "Expected at least 2 timers, got {}", ready.len());
}
