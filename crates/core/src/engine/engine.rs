...
use crate::adapters::{MergeResult, RepoAdapter, SessionAdapter, SessionId};
...
impl<A: Adapters, C: Clock> Engine<A, C> {
    ...
    pub async fn handle_stuck_task(&mut self, task_id: &TaskId) -> Result<(), EngineError> {
        use crate::engine::recovery::{RecoveryAction, RecoveryConfig, RecoveryState};
        ...
        match action {
            RecoveryAction::Nudge => {
                ...
                let recovery = self.recovery_states.get_mut(task_id).unwrap();
    ...
    pub async fn poll_sessions(&mut self) -> Result<(), EngineError> {
        let session_ids: Vec<_> = self.tasks
            .values()
    ...
    pub async fn start_phase_task(&mut self, pipeline_id: &PipelineId) -> Result<TaskId, EngineError> {
