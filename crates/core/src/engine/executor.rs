...
use crate::adapters::{IssueAdapter, RepoAdapter, SessionAdapter};
    ...
    pub async fn execute(&self, effect: Effect) -> Result<(), ExecutorError> {
        match effect {
            ...
            Effect::SpawnSession { name, cwd, command } => {
                self.adapters.sessions().spawn(&name, &cwd, &command).await?;
                Ok(())
            }
            ...
            Effect::Merge {
                ...
                self.adapters.repos().merge(&path, &branch, strategy).await?;
                Ok(())
            }
