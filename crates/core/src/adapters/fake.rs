...
use super::notify::{Notification, NotifyAdapter, NotifyError};
use super::traits::*;
...
#[allow(dead_code)] // Test infrastructure: stores session data for verification
struct FakeSession {
    id: SessionId,
    name: String,
    cwd: PathBuf,
...
impl FakeAdapters {
    ...
    /// Get the notify adapter
    pub fn notify(&self) -> FakeNotifier {
        self.notifier.clone()
    }
...
impl SessionAdapter for FakeSessionAdapter {
    ...
    async fn kill(&self, id: &SessionId) -> Result<(), SessionError> {
        ...
        state
            .calls
            .push(AdapterCall::KillSession { id: id.0.clone() });
    ...
    async fn capture_pane(&self, id: &SessionId, lines: u32) -> Result<String, SessionError> {
        ...
        Ok(state.pane_content.get(&id.0).cloned().unwrap_or_default())
    }
...
impl IssueAdapter for FakeIssueAdapter {
    ...
    async fn get(&self, id: &str) -> Result<IssueInfo, IssueError> {
        ...
        state
            .calls
            .push(AdapterCall::GetIssue { id: id.to_string() });
...
/// Fake notifier that records all calls
...
impl FakeNotifier {
    ...
    /// Check if a notification with the given title was sent
    pub fn was_notified(&self, title: &str) -> bool {
        self.calls
            .lock()
            .unwrap()
            .iter()
            .any(|c| c.notification.title == title)
    }
...
impl NotifyAdapter for FakeNotifier {
    async fn notify(&self, notification: Notification) -> Result<(), NotifyError> {
        if *self.should_fail.lock().unwrap() {
            return Err(NotifyError::Failed("fake failure".to_string()));
        }

        self.calls.lock().unwrap().push(NotifyCall { notification });
        Ok(())
    }
}
