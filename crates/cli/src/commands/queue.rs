...
impl fmt::Display for QueueItemInfo {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        let data_str: Vec<_> = self.data.iter().map(|(k, v)| format!("{}={}", k, v)).collect();
        write!(
            f,
            "{:<20} pri={:<3} attempts={:<2} {}",
            self.id,
...
async fn add_to_queue(
    ...
    let queue = store.load_queue(&name).unwrap_or_else(|_| Queue::new(&name));

    let data_map: HashMap<String, String> = data.into_iter().collect();
    let item_id = format!("{}-{}", name, uuid::Uuid::new_v4().to_string().split('-').next().unwrap());
