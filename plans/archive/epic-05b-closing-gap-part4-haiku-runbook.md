...
Document manual testing procedures for validating `oj` against real Claude (haiku model). Run these tests after major changes to verify the simulator accurately models real behavior.

This phase also provides opportunity to document:
...
1. **`docs/TEST_HAIKU_RUNBOOK.md`** - Manual validation procedures
2. **`docs/PIPELINE_STATE_MACHINE.md`** (optional) - State machine documentation
3. **`docs/FAILURE_MODES.md`** (optional) - Failure catalog and recovery
...
## Optional: State Machine Documentation

During manual testing, document the observed state machine for `docs/PIPELINE_STATE_MACHINE.md`:

### Pipeline Phases

```
┌─────────┐
│  init   │ ← Pipeline created, Claude working
└────┬────┘
     │ oj done
     ▼
┌─────────┐
│  build  │ ← Building/implementing
└────┬────┘
     │ oj done
     ▼
┌─────────┐
...
|------|-----|---------|-------|
| init | build | `oj done` | Claude completed initial task |
| init | failed | `oj done --error` | Claude encountered error |
| any | stuck | timeout | No heartbeat for N seconds |

### Session States

| State | Meaning |
|-------|---------|
| running | tmux session active, Claude working |
| completed | Claude finished, awaiting signal |
| dead | Session terminated unexpectedly |
| none | No session (between phases) |

---

## Optional: Failure Modes Documentation

During testing, catalog failure modes for `docs/FAILURE_MODES.md`:

### Failure Categories

1. **Agent Failures** (most common)
   - Agent stuck in loop
   - Agent ignores CLAUDE.md instructions
   - Agent doesn't signal `oj done`
   - Agent produces wrong output
   - Agent crashes mid-task
   - Agent exhausts context window
   - Agent rate limited by API
...
- [ ] State machine diagram opportunity documented
- [ ] Failure modes catalog opportunity documented
